...
@router.callback_query(lambda F_analytics_call: F_analytics_call.data == "F_analytics")
...
@router.callback_query(lambda F_on_expenses_call: F_on_expenses_call.data == "F_on_expenses")
...
@router.callback_query(FAnalysisExpensesCallback.filter(F.call_analysis == 'F_call_analysis'))
async def withdrawal_from_expense(query: CallbackQuery):
    data = query.data.split('.')[0]
    data = data.split(':')[1]
    ACCOUNT_ID.append(int(data))
    connection = family_finance_server.cost_analytics(account_id=data)
    if not connection:
...
async def cost_pagination(query: CallbackQuery):
    ...
    connection = family_finance_server.cost_pagination(account_id=account_id, page=page)
...
@router.callback_query(lambda by_income_call: by_income_call.data == "F_by_income")
...
@router.callback_query(FAnalysisIncomeCallback.filter(F.call_analysis == 'F_call_analysis'))
async def withdrawal_from_income(query: CallbackQuery):
    data = query.data.split('.')[0]
    data = data.split(':')[1]
    ACCOUNT_ID.append(int(data))
    connection = family_finance_server.revenue_analytics(account_id=data)
    if not connection:
...
async def income_pagination(query: CallbackQuery):
    ...
    for account_id in ACCOUNT_ID:
        account_id = account_id
    connection = family_finance_server.income_pagination(account_id=account_id, page=page)
    for json_list in connection['results']:
